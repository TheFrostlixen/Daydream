...
public class CursorParticles : MonoBehaviour {
	...
	void Update () 
	{
		...
		if (Physics.Raycast (ray, out hit, 100)) {
			...
			// Spawning particles for terrain
			if (hit.collider.name.Equals ("Terrain")) 
			{
				...
				if(DisplayData.attention1 < 68 && DisplayData.attention1 > 60){
					float lerp = Mathf.PingPong(Time.time, duration) / duration;
				...
				if(DisplayData.attention1 <= 100 && DisplayData.attention1 >= 68)
				{		
					float lerp = Mathf.PingPong(Time.time, duration) / duration;
				...
				if(DisplayData.meditation1 < 70 && DisplayData.meditation1 > 60)
				{
					float lerp = Mathf.PingPong(Time.time, duration) / duration;
				...
				if(DisplayData.meditation1 <= 100 && DisplayData.meditation1 >= 70)
				{		
					float lerp = Mathf.PingPong(Time.time, duration) / duration;
			...
			// Spawning particles for flowers
			else
			{
				//showing meditation
				Plant.startColor = colorCurrentMeditation;
				ParticleSystem particleCloneP = (ParticleSystem)Instantiate (Plant, hit.point, Quaternion.LookRotation(hit.normal));
				Destroy(particleCloneP.gameObject, duration);
			}
		}
	}
}
